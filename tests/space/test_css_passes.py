...
def test_fetch_css_tle_lock_recheck_finds_fresh_cache_from_concurrent_winner(monkeypatch):
    """A concurrent location job may refresh the shared TLE while this thread waits
    on _CSS_TLE_FETCH_LOCK; the post-lock re-check must return that TLE without
    touching the network, instead of re-entering the fetch loop."""
    svc = mod.CSSPassService(45.5, -73.5, 10, "UTC")
    calls = {"n": 0}

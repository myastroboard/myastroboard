...
- Meteor-shower radiant visibility is evaluated during the observer's night instead of at a fixed noon-UTC instant; planetary conjunction/appulse minima are refined between samples (with finer Moon sampling); event lists are ordered by true instant so they stay correct across daylight-saving offset changes.
- Equinox and solstice instants are now correct. They were derived from the Sun's declination as reported in GCRS, whose equator is still the J2000 one, so precession since 2000 placed every season about nine hours late (the 2026 March equinox came out at 23:40 UTC instead of 14:46). They are now found from the Sun's apparent ecliptic longitude in the true equinox of date - 0/90/180/270 degrees - and match published times to within the one-minute search resolution. The same pass replaced the per-timestep ephemeris scan with a vectorised one, cutting a full 365-day phenomena refresh from about 1,800 solar-position calls to about 350.
- The zodiacal-light check no longer floods the server log. Its elongation test compared coordinates across two frames that are not related by a pure rotation, so Astropy emitted a warning on every scanned day - roughly 180 lines per refresh. The Sun is now converted into the sampling frame first; the computed values are unchanged.
- The eclipse lookups no longer print ERFA "dubious year" and IERS out-of-range warnings on every cache cycle. The next solar eclipse *visible from a given site* can be many years out - for a Hawaii preset it is in 2031 - and the leap-second and Earth-orientation tables are only published about a year ahead, so Astropy warned that it was assuming UT1-UTC = 0. Nothing was wrong and no newer table exists: the eclipse instant comes from astronomy-engine, and the missing corrections shift only the derived altitude/azimuth, by well under 0.01 degrees. Those two specific warnings are now muted around the eclipse lookups alone, so degraded-accuracy warnings from any other calculation still reach the log.
...
- ISS/CSS solar/lunar transit detection is now vectorised. It previously propagated every geometric pass second-by-second in a Python loop (~150k Skyfield calls, ~5 minutes for a single 20-day report); it now scans each pass in one batched Skyfield call, rejects passes where the station never approaches the Sun/Moon, and only refines a promising pass - about a 30× speed-up with the same results, which matters a lot for multi-location installs where the pass jobs run per location every scheduler cycle.

...
- Meteor-shower radiant visibility is evaluated during the observer's night instead of at a fixed noon-UTC instant; planetary conjunction/appulse minima are refined between samples (with finer Moon sampling); event lists are ordered by true instant so they stay correct across daylight-saving offset changes, and an event whose timestamp cannot be read is now logged rather than quietly parked at the end of the list where it would pass for a distant future event.
- Equinox and solstice instants are now correct. They were measured against the J2000 equator, which precession has since moved, so every season came out about nine hours late (the 2026 March equinox at 23:40 UTC instead of 14:46). They now follow the Sun's apparent ecliptic longitude in the equinox of date and match published times to the minute. The same pass vectorised the search, cutting a 365-day refresh from ~1,800 solar-position calls to ~350.
...
- The eclipse lookups no longer print ERFA "dubious year" and IERS out-of-range warnings on every cache cycle. The next solar eclipse visible from a given site can be years out (2031 for a Hawaii preset), past the leap-second and Earth-orientation tables, which only publish about a year ahead. Nothing was wrong - the missing corrections shift the derived altitude/azimuth by under 0.01 degrees - so those two warnings are muted around the eclipse lookups alone; every other calculation still reports degraded accuracy.

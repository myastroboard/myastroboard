            ...
            try:
                    ...
                    events.append(
                        {
                            'event_type': 'Equinox',
                            'title': self._t(
                                'events_api.special_phenomena.spring_equinox_title', 'Vernal Equinox (Spring)'
                            ),
                            'description': self._t(
                ...
                if float(start_date.jd) <= float(summer_sol.jd) < float(end_date.jd):  # type: ignore
                    ...
                    events.append(
                        {
                            'event_type': 'Solstice',
                            'title': self._t('events_api.special_phenomena.summer_solstice_title', 'Summer Solstice'),
                            'description': self._t(
                    ...
                    events.append(
                        {
                            'event_type': 'Equinox',
                            'title': self._t(
                                'events_api.special_phenomena.autumn_equinox_title', 'Autumnal Equinox (Fall)'
                            ),
                            'description': self._t(
                ...
                if float(start_date.jd) <= float(winter_sol.jd) < float(end_date.jd):  # type: ignore
                    ...
                    events.append(
                        {
                            'event_type': 'Solstice',
                            'title': self._t('events_api.special_phenomena.winter_solstice_title', 'Winter Solstice'),
                            'description': self._t(
                    ...
                    if sun_alt < -6:  # End of twilight
                        ...
                        if ecliptic_alt > 20:
                            ...
                            if is_moon_ok:
                                ...
                                viewing_type_localized = (
                                    self._t('events_api.special_phenomena.zodiacal_viewing_evening', 'Evening')
                                    if is_spring
                                    else self._t('events_api.special_phenomena.zodiacal_viewing_morning', 'Morning')
                                )
                                events.append(
                                    {
    ...
    def _find_milky_way_core_visibility(self, start_date: Time, end_date: Time) -> List[Dict[str, Any]]:
        ...
        while current_date <= end_local_date:
            try:
                ...
                check_dt = datetime(current_date.year, current_date.month, current_date.day, 2, 0, 0, tzinfo=tz)
                ...
                title = self._t('events_api.special_phenomena.milky_way_title', 'Milky Way Core Visible')
                description = self._t(

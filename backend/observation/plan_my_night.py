...
def _observable_runs(
    ...
    for i, m in enumerate(margins):
        ...
        if observable and run_start is None:
            run_start = _crossing(i - 1, i) if prev_margin is not None and prev_margin < 0 else times[i]
        elif not observable and run_start is not None:
            run_end = _crossing(i - 1, i) if prev_margin is not None else times[i - 1]
